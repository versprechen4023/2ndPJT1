...
public class WareHouseServiceImpl implements WareHouseService {
	...
	public boolean whInsert(WareHouseinsertDTO warehouseinsertDTO) {
		...
		if (getNum == null) {
			...
			// getNum.replaceAll("[^0-9]", ""): 이 부분은 getNum 문자열에서 숫자를 제외한 모든 문자를 빈 문자열로
			// 대체합니다.
		...
		// 고유 번호 저장
		////////////////////////////////////////////////////////////////////// 설명
		// warehouseinsertDTO.getType(): 이 부분은 warehouseinsertDTO 객체에서 getType() 메서드를
		// 호출하여 type 속성을 가져옵니다.
		// 이 값은 이전에 설정한 문자열 중 하나일 것으로 예상됩니다.

		// numStr: 이 부분은 이전에 생성한 numStr 문자열입니다. 이 문자열은 숫자를 4자리로 포맷한 문자열입니다.

		// warehouseinsertDTO.getType()+numStr: 이 부분은 warehouseinsertDTO 객체의 type과
		// numStr을 연결하여 prod_code 속성을 설정합니다.
	...
	public boolean whDelete(List<Map<String, String>> codeAndTypeList) {
		...
		return wareHouseDAO.whDelete(codeAndTypeList);
